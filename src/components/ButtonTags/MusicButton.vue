...
<template lang="">
  <div>
     ...
     <v-btn
        ...
        title="Выбирайте новости по своим предпочтениям: язык,теги  и время постинга"
    ...
    <v-card-text>
      <v-card title="" class="bg-primary">
        <v-card-text>
          ...
          <br />
            <br />
            <h4 class="text-center text-white">Тег : Музыка</h4>
            <br />
            <br />
            <div>
              <VueDatePicker
...
export default {
components: { dat, dat2, hosting, logo,  cinema },
data() {
  return {
    selectedIdsDisplay: [],
    selectedIds: [],
  ...
  async getData() {
    ...
    try {
      const response = await fetch(
        `https://api-epicnews404.azurewebsites.net/Articles/TopHeadlines?SiteId=1&WithImageOnly=true&CategoryId=8&Language=14&Page=1&PageSize=${pageSize}`
      );
...
const selectedIdsMessage = `Удалить из постинга: ${this.selectedIds.join(", ")}`;
const tagsMessage = data.tags ? `Теги: ${data.tags.join(", ")}` : '';
const telegramMessage = `
${selectedIdsMessage}
${tagsMessage}
Название канала: ${data.channelname}
Дата и время публикации новостей: ${this.formatDateTime(data.dateTime)}
`;
